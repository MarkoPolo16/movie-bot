...
class RatingView(discord.ui.View):
    ...
    async def save_rating(self, interaction: discord.Interaction, rating: float):
        try:
            ...
            # XP-FARMING SCHUTZ: Prüfen, ob bereits bewertet
            ...
            # 2. XP hinzufügen (nur wenn noch nicht bewertet)
            level_up = False
            if xp_gain > 0:
                cursor.execute("""
                    INSERT INTO levels (user_id, xp, level) 
                ...
                if level_up:
                    log_chan = bot.get_channel(LEVEL_LOG_CHANNEL_ID)
                    if log_chan: 
                        await log_chan.send(f"🎉 {interaction.user.mention} has reached Level **{level}** through a rating!")
...
async def tv_info(interaction: discord.Interaction, tv_name: str):
    ...
    try:
        ...
        # 3. Get Server Average from Database
        conn = psycopg2.connect(DATABASE_URL)
        cursor = conn.cursor()
...
@bot.tree.command(name="top10tv", description="Show the server's top 10 TV shows")
async def top10tv(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=False)
    ...
    try:
        ...
        # Abfrage: Sortierung nach Bewertung und Anzahl der Votes für TV-Serien
        cursor.execute("""
            SELECT tv_title, AVG(rating), COUNT(*) 
            FROM tv_ratings 
            GROUP BY tv_title 
            HAVING COUNT(*) > 0 
            ORDER BY AVG(rating) DESC, COUNT(*) DESC 
        ...
        if not top_shows:
            return await interaction.followup.send("No TV show ratings found yet.")
        
        # Embed erstellen
        embed = discord.Embed(title="🏆 Top 10 TV Shows", color=CYAN)
        
        description = ""
        for i, (title, avg, count) in enumerate(top_shows, 1):
        ...
        print(f"Error in top10tv: {e}")
        await interaction.followup.send("An error occurred while fetching the top TV show list.")
