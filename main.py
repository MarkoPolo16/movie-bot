...
# ==========================================
# MOVIE RATING VIEW (0.5 FIX INCLUDED)
# ==========================================
class RatingView(discord.ui.View):
    ...
    async def handle_rating(self, interaction, rating):
        ...
        embed.add_field(name="⭐ Avg", value=f"{avg}/5")
        embed.add_field(name="👤 Your", value=f"{rating}/5")
...
async def search(interaction: discord.Interaction, movie_name: str):
    ...
    """, (movie["id"], str(interaction.user.id)))
    ...
    embed = discord.Embed(title=movie["title"], color=CYAN)
    embed.add_field(name="⭐ Avg", value=f"{avg}/5")

    # FIX: nur eigener User sieht "My Rating"
    if user:
        embed.add_field(name="👤 Your Rating", value=f"{user[0]}/5")
